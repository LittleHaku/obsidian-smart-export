...
- **Blocking**: use GitHub's native issue dependencies. Where dependencies aren't available, add `Blocked by: #<n>, #<n>` at the top of the child body.
- **Frontier query**: list the map's open children, then exclude issues with an open blocker or an assignee. The first remaining issue in map order wins.
